    ...
    stages {
        stage("run qa regression tests") {
            ...
            steps {
                ...
                sh "/usr/local/bin/docker build --platform linux/amd64 -t qa-regression -f Dockerfile.reg ."
                sh "/usr/local/bin/docker run --platform linux/amd64 --add-host localhost:127.0.0.1 --name qa-regression-pod qa-regression"
